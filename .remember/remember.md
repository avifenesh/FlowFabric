...
- **#6 Value::ServerError**: PARTIAL. Top-level errors: parser returns `Err(ValkeyError)` directly. Cluster pipeline `NodeResponse = ValkeyResult<Value>`. `Value::ServerError` variant stays — needed in two places: (1) parser stores error array elements as `Value::ServerError(e)` (EXEC/RESP3), (2) standalone pipeline accumulator (`multiplexed.rs`) stores non-transaction per-command errors as `Value::ServerError(e)` in the buffer to preserve `raise_on_error=false` semantics — controlled by `is_transaction` flag (transactions extract first_err, pipelines keep errors in buffer). Full removal deferred: requires `Value::Array` → `Vec<ValkeyResult<Value>>` + `FromValkeyValue` changes.
