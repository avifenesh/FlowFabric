//! Round-2 cluster+TLS — ferriskey 50/50 GET/SET. Mirror of
//! ferriskey_50_50.rs over cluster+TLS; `{wider}` single-slot.
...
const SCENARIO: &str = "cluster_wider_50_50";
...
async fn main() -> Result<()> {
    ...
    eprintln!("[cluster-ferriskey-50-50] {}", env.describe());
    ...
    eprintln!("[cluster-ferriskey-50-50] warmup {}s + run {}s, {} workers", args.warmup_secs, args.duration_secs, args.workers);
    barrier.wait().await;
    let t0 = Instant::now();
    ...
    let path = cluster_shared::write_cluster_report(
        ...
        "ferriskey cluster+TLS = 50/50 GET/SET on 4 KiB, {wider} single-slot.",
    )?;
    eprintln!("[cluster-ferriskey-50-50] wrote {} — {:.1} ops/sec p50={:.3}ms p99={:.3}ms errs={}",
