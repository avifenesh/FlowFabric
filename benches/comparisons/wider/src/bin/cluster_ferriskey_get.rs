//! Round-2 cluster+TLS — ferriskey 100% GET.
...
async fn main() -> Result<()> {
    ...
    eprintln!("[cluster-ferriskey-get] {}", env.describe());
    ...
    eprintln!("[cluster-ferriskey-get] warmup {}s + run {}s, {} workers", args.warmup_secs, args.duration_secs, args.workers);
    barrier.wait().await;
    let t0 = Instant::now();
    ...
    let path = cluster_shared::write_cluster_report(
        ...
        "ferriskey cluster+TLS = 100% GET on 4 KiB pre-seeded, {wider} single-slot.",
    )?;
    eprintln!("[cluster-ferriskey-get] wrote {} — {:.1} ops/sec p50={:.3}ms p99={:.3}ms errs={}",
