//! Round-2 cluster+TLS — ferriskey 100-cmd GET pipeline. Single slot
//! via `{wider}` tag — cluster pipelines require all keys hash to
//! the same slot. Cross-slot pipelines are out of scope for round 2
//! (report §4).
...
async fn main() -> Result<()> {
    ...
    eprintln!("[cluster-ferriskey-pipeline] {}", env.describe());
    ...
    eprintln!("[cluster-ferriskey-pipeline] warmup {}s + run {}s, {} workers, {}-cmd pipe",
    ...
    let path = cluster_shared::write_cluster_report(
        ...
        "ferriskey cluster+TLS = TypedPipeline of 100 GETs, {wider} single-slot.",
    )?;
    eprintln!("[cluster-ferriskey-pipeline] wrote {} — {:.1} ops/sec ({:.1} pipe/s) p50={:.3}ms p99={:.3}ms errs={}",
