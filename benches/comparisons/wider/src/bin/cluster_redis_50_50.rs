//! Round-2 cluster+TLS — redis-rs 50/50 GET/SET.
...
async fn main() -> Result<()> {
    ...
    eprintln!("[cluster-redis-50-50] {}", env.describe());
    ...
    eprintln!("[cluster-redis-50-50] warmup {}s + run {}s, {} workers", args.warmup_secs, args.duration_secs, args.workers);
    barrier.wait().await;
    let t0 = Instant::now();
    ...
    let path = cluster_shared::write_cluster_report(
        ...
        "redis-rs cluster+TLS = 50/50 GET/SET on 4 KiB, {wider} single-slot.",
    )?;
    eprintln!("[cluster-redis-50-50] wrote {} — {:.1} ops/sec p50={:.3}ms p99={:.3}ms errs={}",
