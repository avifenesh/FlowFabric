//! Round-2 cluster+TLS — redis-rs 100% GET.
...
const SCENARIO: &str = "cluster_wider_get_only";
...
async fn main() -> Result<()> {
    ...
    eprintln!("[cluster-redis-get] {}", env.describe());
    ...
    eprintln!("[cluster-redis-get] warmup {}s + run {}s, {} workers", args.warmup_secs, args.duration_secs, args.workers);
    barrier.wait().await;
    let t0 = Instant::now();
    ...
    let config = serde_json::json!({
        ...
        "warmup_s": args.warmup_secs, "mix": "100% GET",
    });
    let path = cluster_shared::write_cluster_report(
        &args.results_dir, SCENARIO, SYSTEM, config, &env,
        wall, total_ops, total_errors, &lat,
        "redis-rs cluster+TLS = 100% GET on 4 KiB pre-seeded, {wider} single-slot.",
    )?;
    eprintln!("[cluster-redis-get] wrote {} — {:.1} ops/sec p50={:.3}ms p99={:.3}ms errs={}",
