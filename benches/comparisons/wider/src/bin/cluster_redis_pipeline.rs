//! Round-2 cluster+TLS — redis-rs 100-cmd GET pipeline, single-slot.
...
const SCENARIO: &str = "cluster_wider_pipeline_100";
...
async fn main() -> Result<()> {
    ...
    eprintln!("[cluster-redis-pipeline] {}", env.describe());
    ...
    eprintln!("[cluster-redis-pipeline] warmup {}s + run {}s, {} workers, {}-cmd pipe",
    ...
    let path = cluster_shared::write_cluster_report(
        ...
        "redis-rs cluster+TLS = pipe() of 100 GETs, {wider} single-slot.",
    )?;
    eprintln!("[cluster-redis-pipeline] wrote {} — {:.1} ops/sec ({:.1} pipe/s) p50={:.3}ms p99={:.3}ms errs={}",
...
async fn drive_worker(
    wi: usize, mut conn: ClusterConnection, keys: Arc<Vec<String>>,
...
async fn do_one(conn: &mut ClusterConnection, keys: &[String], rng: &mut StdRng) -> Result<()> {
