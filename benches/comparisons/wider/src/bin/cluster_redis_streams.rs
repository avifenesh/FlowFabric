//! Round-2 cluster+TLS — redis-rs streams (XADD + XRANGE).
...
async fn main() -> Result<()> {
    ...
    eprintln!("[cluster-redis-streams] {}", env.describe());
    ...
    let client = build_redis_cluster(&env)?;
    {
        ...
        for wi in 0..args.workers {
            ...
            for _ in 0..50 {
                let _: redis::Value = redis::cmd("XADD").arg(&key).arg("*").arg("payload").arg(&payload[..])
                    .query_async(&mut seeder).await.context("pre-seed XADD")?;
            }
        }
    }
    ...
    for wi in 0..args.workers {
        ...
        handles.push(tokio::spawn(async move {
            drive_worker(wi, conn, payload, barrier, warmup, run).await
        }));
    }
    eprintln!("[cluster-redis-streams] warmup {}s + run {}s, {} workers, 1:4 XADD:XRANGE",
    ...
    let path = cluster_shared::write_cluster_report(
        ...
        "redis-rs cluster+TLS = 20% XADD + 80% XRANGE-10, per-worker {wider} streams.",
    )?;
    eprintln!("[cluster-redis-streams] wrote {} — {:.1} ops/sec p50={:.3}ms p99={:.3}ms errs={}",
...
async fn drive_worker(
    wi: usize, mut conn: ClusterConnection, payload: Vec<u8>,
    ...
    while Instant::now() < warmup_end {
        let _ = do_one(&mut conn, &key, &payload, &mut rng).await;
    }
    barrier.wait().await;
    let run_end = Instant::now() + run;
    while Instant::now() < run_end {
        let t0 = Instant::now();
        match do_one(&mut conn, &key, &payload, &mut rng).await {
            Ok(_) => {
...
async fn do_one(conn: &mut ClusterConnection, key: &str, payload: &[u8], rng: &mut StdRng) -> Result<()> {
    if rng.random_ratio(20, 100) {
        let _: redis::Value = redis::cmd("XADD").arg(key).arg("*").arg("payload").arg(payload)
            .query_async(conn).await.context("XADD")?;
    } else {
        let _: redis::Value = redis::cmd("XRANGE").arg(key).arg("-").arg("+").arg("COUNT").arg(XRANGE_COUNT as u64)
            .query_async(conn).await.context("XRANGE")?;
    }
    Ok(())
}
