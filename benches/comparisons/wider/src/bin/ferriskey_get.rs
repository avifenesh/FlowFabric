//! Wider perf investigation — ferriskey 100% GET.
//!
//! Read-heavy extreme of the 80/20 series. Zero writes in the hot
//! loop so server-side effects are pure GET bandwidth; any client
//! delta here is read-reply parsing + dispatch overhead only. If
//! 80/20 ties and this also ties, client-side command-type overhead
//! is flat across read-shape workloads.
...
async fn main() -> Result<()> {
    ...
    eprintln!(
        "[wider-ferriskey-get] warmup {}s + run {}s, {} workers",
    ...
    let path = shared::write_report(
        ...
        "ferriskey wider = ClientBuilder per worker + 100% GET on 4 KiB pre-seeded values.",
    )?;
    eprintln!(
        "[wider-ferriskey-get] wrote {} — {:.1} ops/sec p50={:.3}ms p99={:.3}ms errs={}",
        path.display(),
