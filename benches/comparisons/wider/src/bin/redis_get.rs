//! Wider perf investigation — redis-rs 100% GET. Mirror of ferriskey_get.rs.
...
const SCENARIO: &str = "wider_get_only";
...
async fn main() -> Result<()> {
    ...
    eprintln!(
        "[wider-redis-get] warmup {}s + run {}s, {} workers",
    ...
    let path = shared::write_report(
        ...
        "redis-rs wider = MultiplexedConnection per worker + 100% GET on 4 KiB pre-seeded.",
    )?;
    eprintln!(
        "[wider-redis-get] wrote {} — {:.1} ops/sec p50={:.3}ms p99={:.3}ms errs={}",
        path.display(),
