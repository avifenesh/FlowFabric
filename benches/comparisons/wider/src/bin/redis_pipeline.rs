//! Wider perf investigation — redis-rs 100-command GET pipeline.
//! Mirror of ferriskey_pipeline.rs. Uses `redis::pipe()` + `.query_async()`.
...
async fn main() -> Result<()> {
    ...
    for wi in 0..args.workers {
        ...
        handles.push(tokio::spawn(async move {
            drive_worker(wi, conn, keys, barrier, warmup, run).await
        }));
    }
    eprintln!(
        "[wider-redis-pipeline] warmup {}s + run {}s, {} workers, {}-cmd pipe",
    ...
    let path = shared::write_report(
        ...
        "redis-rs wider = redis::pipe() of 100 GETs per iteration, same {wider} hash tag. \
         Throughput counts per-GET (pipelines × 100). Latency = per-pipeline = per-100-ops-amortised.",
    )?;
    eprintln!(
        "[wider-redis-pipeline] wrote {} — {:.1} ops/sec ({:.1} pipe/s) p50={:.3}ms p99={:.3}ms errs={}",
        path.display(),
...
async fn drive_worker(
    wi: usize, mut conn: MultiplexedConnection, keys: Arc<Vec<String>>,
    ...
    while Instant::now() < warmup_end {
        let _ = do_one(&mut conn, &keys, &mut rng).await;
    }
    barrier.wait().await;
    let run_end = Instant::now() + run;
    while Instant::now() < run_end {
        let t0 = Instant::now();
        match do_one(&mut conn, &keys, &mut rng).await {
            Ok(_) => {
...
async fn do_one(conn: &mut MultiplexedConnection, keys: &[String], rng: &mut StdRng) -> Result<()> {
    // Build a non-atomic pipeline of PIPELINE_SIZE GETs, fire, await
    // the full reply vector. Matches the ferriskey side's
    // TypedPipeline.execute() semantics.
    let mut pipe = redis::pipe();
    for _ in 0..PIPELINE_SIZE {
        let k = &keys[rng.random_range(0..keys.len())];
        pipe.get(k);
    }
    let _: Vec<Option<Vec<u8>>> = pipe.query_async(conn).await.context("pipeline query_async")?;
    Ok(())
}
