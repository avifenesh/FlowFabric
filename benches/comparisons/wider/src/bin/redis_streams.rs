//! Wider perf investigation — redis-rs streams (XADD + XRANGE).
//! Mirror of ferriskey_streams.rs.
...
async fn main() -> Result<()> {
    ...
    eprintln!(
        "[wider-redis-streams] warmup {}s + run {}s, {} workers, 1:4 XADD:XRANGE",
    ...
    let path = shared::write_report(
        ...
        "redis-rs wider = streams = 20% XADD + 80% XRANGE-10 per worker. Same per-worker stream \
         key under {wider}; pre-seeded 50-entry backlog.",
    )?;
    eprintln!(
        "[wider-redis-streams] wrote {} — {:.1} ops/sec p50={:.3}ms p99={:.3}ms errs={}",
        path.display(),
...
async fn drive_worker(
    wi: usize, mut conn: MultiplexedConnection, payload: Vec<u8>,
...
async fn do_one(conn: &mut MultiplexedConnection, key: &str, payload: &[u8], rng: &mut StdRng) -> Result<()> {
    if rng.random_ratio(20, 100) {
        let _: redis::Value = redis::cmd("XADD")
            .arg(key).arg("*").arg("payload").arg(payload)
            .query_async(conn).await.context("XADD")?;
    } else {
        let _: redis::Value = redis::cmd("XRANGE")
