...
mod tests {
    ...
    fn from_code_roundtrip() {
        // All 48 Lua error codes must parse successfully
        let codes = [
    ...
    #[test]
    fn all_error_codes_parseable() {
        // Every variant that can come from Lua should be parseable
        let all_codes = [
            "stale_lease",
            "lease_expired",
            "lease_revoked",
            ...
            "use_claim_resumed_execution",
            "not_a_resumed_execution",
            ...
            "payload_too_large",
            "signal_limit_exceeded",
            "invalid_waitpoint_key",
            "invalid_lease_for_suspend",
            "resume_condition_not_met",
            "execution_not_terminal",
            "max_replays_exhausted",
            "stream_closed",
            ...
            "budget_override_not_allowed",
            "quota_policy_not_found",
            "rate_limit_exceeded",
            "concurrency_limit_exceeded",
            "quota_attach_conflict",
            "invalid_quota_spec",
        ];
        for code in all_codes {
            assert!(
                ScriptError::from_code(code).is_some(),
                "failed to parse: {}",
                code
            );
        }
