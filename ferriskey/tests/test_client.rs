...
pub(crate) mod shared_client_tests {
    ...
    async fn create_client(server: &BackingServer, configuration: TestConfiguration) -> Client {
        match server {
            BackingServer::Standalone(server) => {
                ...
                // still isn't available to connection. This should be fixed in [ValkeyServer].
                retry(|| async {
                    Client::new(
    ...
    fn test_pipeline_resp_support(
        ...
        block_on_all(async {
            ...
            pipeline.cmd("HSET").arg(&key).arg(&field).arg(&value);
            pipeline.cmd("HSET").arg(&key).arg(&field2).arg(&value2);
    ...
    fn test_pipeline_with_blocking_command_returns_null(#[values(false, true)] use_cluster: bool) {
        block_on_all(async move {
            ...
            let mut pipeline = Pipeline::new();
            pipeline.cmd("SET").arg(&key).arg("value1");
            pipeline.cmd("GET").arg(&key);
            pipeline.cmd("BLPOP").arg(&key2).arg(1.0);
            pipeline.cmd("GET").arg(&key2);
    ...
    fn test_parallel_pipeline_and_kill_connection(#[values(false, true)] use_cluster: bool) {
        block_on_all(async {
            ...
            let pipeline_future = async {
                let mut pipeline = Pipeline::new();
                pipeline.cmd("BLPOP").arg(&key).arg(2.0);
    ...
    // Telemetry counter tests deleted in the telemetry-redesign:
    // `ferriskey::Telemetry` is now a deprecated compatibility stub whose
    // methods no-op (see `ferriskey/src/telemetry_compat.rs`). Connection
    // and client lifecycle are observable via tracing events
    // (`target = "ferriskey"`, `event = "connection_opened" | "client_created"
    // | "connection_closed" | ...`) on the standard tracing subscriber
    // pipeline. A replacement integration test that captures and asserts
    // on those events lives at `tests/test_tracing_spans.rs`.
    ...
    fn test_request_transaction_and_convert_all_values(#[values(false, true)] use_cluster: bool) {
        block_on_all(async {
            ...
            pipeline.cmd("HGETALL").arg(&key);
            pipeline.cmd("HEXISTS").arg(&key).arg("bar");
            pipeline.cmd("DEL").arg(&key);
            pipeline.cmd("SET").arg(&key).arg("0");
