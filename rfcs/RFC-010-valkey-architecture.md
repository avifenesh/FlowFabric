...
| 22 | `apply_dependency_to_child` | RFC-007 | Create dependency record on child partition, increment unsatisfied count, set eligibility=blocked_by_dependencies if needed | A | 4 | exec_core, deps_meta, unresolved_set, dep_hash |
| 23 | `resolve_dependency` | RFC-007 | Mark dependency satisfied or impossible, update counters, if all satisfied: set eligible_now + add to eligible. If impossible: set terminal=skipped + add to terminal | A | 6 | exec_core, deps_meta, unresolved_set, dep_hash, eligible_zset, terminal_zset |
