...
**Defensive heal on `AlreadyMember` return (revision 4, 2026-04-19):** the landed `ff_add_execution_to_flow` implementation stamps `exec_core.flow_id` inside the `SISMEMBER`-already-member branch as well as the freshly-added branch. The write is idempotent by value — if the exec is in this flow's `members_set`, the exec's `flow_id` must already equal this flow's id (or be empty, for an orphan from the pre-phase-3 two-phase-contract era discovered during a rolling upgrade). Re-stamping costs one HSET round-trip but strictly strengthens the invariant: after `ff_add_execution_to_flow` returns successfully on ANY branch, `exec_core.flow_id == args.flow_id` is guaranteed. `node_count` is NOT incremented on this branch, preserving the idempotent-repeat semantics. Documented here rather than silently buried in the implementation because the behaviour widens the RFC §7.3 spec ("single atomic commit of both writes") with a second write case the spec did not name.

**Cross-flow guard (revision 4, 2026-04-19):** to prevent an exec silently belonging to two flows when a caller mistakenly passes a flow_id different from the one the exec already belongs to, the Lua body HGETs `exec_core.flow_id` before the `SADD K.members_set` in step 3 and returns `err("already_member_of_different_flow:<flow_id>")` if the stored value is non-empty and mismatched. Preserves RFC-007's "exec belongs to at most one flow at a time" invariant; makes the violation loud rather than silent. Only fires on the not-yet-a-member branch (step 3); the already-member branch is data-unambiguous by construction of its keying.

**Pre-flight partition match (revision 4, 2026-04-19):** `Server::add_execution_to_flow` adds a pre-FCALL check comparing `exec_partition.index` against `flow_partition.index`. On mismatch it returns `ServerError::PartitionMismatch(...)` with a message that names both indices and points the caller at `ExecutionId::for_flow(&flow_id, config)`. This turns a raw Valkey CROSSSLOT / Lua-lib error into a typed, actionable caller-side signal when the consumer contract from §7.3 ("mint via `for_flow`") is violated. No new error variant; uses the existing `PartitionMismatch(String)` variant from `ServerError`.
